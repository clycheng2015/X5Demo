...
export default {
    get(url:String, name: String): Promise<Object|String> {
        return NativeModules.X5RNCookieManager.getCookie(url).then((value: String): Object => {
            if (name && value) {
    ...
    set(url:String, name: String, value: any, options?: Object): Promise {
        return NativeModules.X5RNCookieManager.setCookie(url, cookie.serialize(name, value, options));
    },

    async clear(url?: String): Promise {
        await NativeModules.X5RNCookieManager.clearCookie(url || null);
        ...
        return data ? new Promise((resolve, reject) => {
            ...
            async function retry() {
                retryCount++;
                await NativeModules.X5RNCookieManager.clearCookie(url);
